...
export default function DashboardPage() {
  ...
  const [adminGuilds, setAdminGuilds] = useState<any>({ known: [] });
    ...
    const fetchAdminGuilds = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const guildIds = data.known.map((guild: any) => guild.id);
          console.log(guildIds);
        } else {
          console.error("Error fetching admin guilds:", data.message);
        }
      } catch (error) {
            ...
            {adminGuilds.known.length > 0 ? (
              <>
                ...
                <div className="flex flex-col items-center gap-6 sm:hidden overflow-y-auto max-h-[80vh] w-full">
                  {adminGuilds.known.map((guild: any, index: any) => (
                    <div
                      key={index}
